...
importance_order <- function(rashomon_set){
  ...
  ref_auc               <- rashomon_set$ref_auc
  rash_auc              <- rashomon_set$rash_auc
  ...
  rash_imp_order        <- cbind(model = model_names, auc = model_auc,  acc = model_acc, rash_imp_order, rash_imp_dif)
  return(rash_imp_order)
}

# importance_order(rasho)
